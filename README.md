...
- `main_parameters` - главные параметры утилиты:
  - `use_coockie=True` - использование `Coockie` файлов [VKontakte](https://vk.com/feed) для скачивания документов.
...
> :warning: Хотя данная опция может помочь в случаях, когда существуют проблемы с сертификатом [VKontakte](https://vk.com/feed), лучше найти причину, по которой скрипт не может с ним работать.
